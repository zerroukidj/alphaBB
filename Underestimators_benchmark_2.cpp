...
double k[5], k_v[5], sig[5], Kq, ksg[5], sigma[5], K1;
...
double fx(double x[5]) {
    ...
    //f18 f = pow((x[1] - 1), 2) + pow((x[2] - 1), 2) + pow((x[1] + 2 * x[2] - 3), 2) + pow((x[1] + 2 * x[2] - 3), 4);
     f= (pow(x[1] , 2) - 0.1 * cos(5 * pii * x[1])) + (pow(x[2] , 2) - 0.1 * cos(5 * pii * x[2]));
...
double fx0(double x1, double x2) {
    ...
    //f18 f = pow((x[1] - 1), 2) + pow((x[2] - 1), 2) + pow((x[1] + 2 * x[2] - 3), 2) + pow((x[1] + 2 * x[2] - 3), 4);
     f= (pow(x1 , 2) - 0.1 * cos(5 * pii * x1)) + (pow(x2 , 2) - 0.1 * cos(5 * pii * x2));
    //f1p f = (cos(2 * x[1] + 1) + 2 * cos(3 * x[1] + 1) + 3 * cos(4 * x[1] + 1) + 4 * cos(5 * x[1] + 1) + 5 * cos(6 * x[1] + 1)) * (cos(2 * x[2] + 1) + 2 * cos(3 * x[2] + 1) + 3 * cos(4 * x[2] + 1) + 4 * cos(5 * x[2] + 1) + 5 * cos(6 * x[2] + 1));
    //f2p f = 1 + pow(sin(x[1]), 2) + pow(sin(x[2]), 2) - 0.1 * exp(-pow(x[1], 2) - pow(x[2], 2));
    //f3p f = 0.5 + (sin(pow(x[1], 2) - pow(x[2], 2)) - 0.5) / pow((1 + 0.001 * (pow(x[1], 2) + pow(x[2], 2))), 2);
    //f4p f = -1 / (pow((x[1] - 4), 2) + pow((x[2] - 4), 2) + 0.1) - 1 / (pow((x[1] - 1), 2) + pow((x[2] - 1), 2) + 0.2) - 1 / (pow((x[1] - 8), 2) + pow((x[2] - 8), 2) + 0.2);
...
double myfunc3(unsigned n, const double* x, double* grad, void* my_func_data)
{
    ...
    return   (pow(x[0], 2) - 0.1 * cos(5 * pii * x[0])) + (pow(x[1], 2) - 0.1 * cos(5 * pii * x[1])) - (0.5) * sum;
}
double myfunc4(unsigned n, const double* x, double* grad, void* my_func_data)
{
    ...
    return  (pow(x[0], 2) - 0.1 * cos(5 * pii * x[0])) + (pow(x[1], 2) - 0.1 * cos(5 * pii * x[1])) - (sigma[1] * (((log(((x[0] - xg[1].lower()) / (xg[1].upper() - xg[1].lower())) + 1) + log(((xg[1].upper() - x[0]) / (xg[1].upper() - xg[1].lower())) + 1)) / log(2)) - 1)) - (sigma[2] * (((log(((x[1] - xg[2].lower()) / (xg[2].upper() - xg[2].lower())) + 1) + log(((xg[2].upper() - x[1]) / (xg[2].upper() - xg[2].lower())) + 1)) / log(2)) - 1));
}
// interval xg
...
double myfunc1(unsigned n, const double* x, double* grad, void* my_func_data)
{
    ...
    return  (pow(x[0], 2) - 0.1 * cos(5 * pii * x[0])) + (pow(x[1], 2) - 0.1 * cos(5 * pii * x[1])) - (0.5) * ((k[1] * ((xg[1].upper() - x[0]) * (x[0] - xg[1].lower()))) + (k[2] * ((xg[2].upper() - x[1]) * (x[1] - xg[2].lower()))));
}
...
double myfunc2(unsigned n, const double* x, double* grad, void* my_func_data)
{
    ...
    return (pow(x[0], 2) - 0.1 * cos(5 * pii * x[0])) + (pow(x[1], 2) - 0.1 * cos(5 * pii * x[1]));
}
...
void local_min(double si[5], int numb) {
    if (numb == 1) {
        ...
        if (nlopt_optimize(opt, xi, &minf) < 0) {
            ...
            printf("found minimum at LB(%g,%g) = %0.10g\n", xi[0], xi[1], minf);
        }
        sbar[1] = xi[0];
        sbar[2] = xi[1];
...
int main()
{
    ...
    std::cout << "AlphaBB underestimator=1; Proposed underestimator=2; Piecewise underestimator=3; Quadratic underestimator=4 \n";
    std::cin >> choice;
    if (choice == 3) {
